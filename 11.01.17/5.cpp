...
int i = left, j = right;											//в массивах объёмов и ценностей вещей
    ...
    if (left < j)  thriceQSort(a, b, c, d, left, j);
    if (i < right)  thriceQSort(a, b, c, d, i, right);
}
...
void Continious_knapsack(int W, int n) {
     double sum = 0.0;			//ответ
      ...
      int *abc = new int [n];		//массив, запоминающий индекс элементов
      float *abc1 = new float [n];	//массив запоминания количества вещей в рюкзаке
